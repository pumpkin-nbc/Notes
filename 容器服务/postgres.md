...
docker run -d \
           ...
           # 建议设置临时密码，部署成功后修改密码 ALTER USER username WITH PASSWORD 'newpassword';
           # 设置数据库root的密码 可以选择使用file文件传入 两个变量二选一即可 POSTGRES_PASSWORD POSTGRES_PASSWORD_FILE
           -e POSTGRES_PASSWORD=mysecretpassword \
           -e POSTGRES_PASSWORD_FILE=/run/secrets/postgres-passwd \
           # 设置默认数据库名称
           -e POSTGRES_DB=postgres \
           # 设置wal日志存放位置
           -e POSTGRES_INITDB_WALDIR=/data/PostgreSQL/wal \
           # 设置数据库位置
           -e PGDATA=/var/lib/postgresql/data
           # 设置自定义数据库配置
           -v /data/docker/postgres/conf/postgresql.conf:/etc/postgresql/postgresql.conf \
           # 持久化保存数据库
