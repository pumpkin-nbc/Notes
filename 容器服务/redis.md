...
# Redis 配置文件示例。
#
# 请注意，为了读取配置文件，Redis 必须以文件路径作为第一个参数开始:
#
# ./redis-server /path/to/redis.conf

# 单位说明:当需要内存大小时, 可以以 1k 5GB 4M 等通常的形式指定它:
#
# 1k => 1000 bytes
# 1kb => 1024 bytes
# 1m => 1000000 bytes
...
# 在此处包含一个或多个其他配置文件。
# 如果您有一个用于所有 Redis 服务器的标准模板，但还需要自定义一些每个服务器的设置，这将非常有用。
# 文件可以包含其他文件，因此请明智地使用它.
#
# 请注意，选项 “include” 不会被 admin 或 Redis Sentinel 的命令 “CONFIG REWRITE” 重写。
# 由于 Redis 始终使用最后处理的行作为配置指令的值，因此您最好将 includes 放在此文件的开头，以避免在运行时覆盖配置更改。
#
# 如果您有兴趣使用 includes 覆盖配置选项，最好使用 include 作为最后一行.
#
# 包含的路径可以包含通配符。所有与通配符匹配的文件都将按字母顺序包含。
# 请注意，如果 include 路径包含通配符，但在服务器启动时没有文件匹配，则 include 语句将被忽略，并且不会发出错误。
# 因此，包含空目录中的通配符文件是安全的。
#
...
# 默认情况下，如果未指定 “bind” 配置指令，Redis 将侦听来自主机上所有可用网络接口的连接。
# 可以使用 “bind” 配置指令，后跟一个或多个 IP 地址来只监听一个或多个选定的接口。
# 每个地址都可以以 “-” 为前缀，这意味着如果地址不可用，redis 不会启动失败。不可用仅指与任何网络接口都不对应的地址。
# 已在使用的地址将始终失败，并且不支持的协议将始终以静默方式跳过。
#
# 例子:
#
...
#
# ~~~ 警告 ~~~
# 如果运行 Redis 的计算机直接暴露在 Internet 上，则绑定到所有接口是危险的，并且会将实例暴露给 Internet 上的每个人。
# 因此，默认情况下，我们取消注释以下 bind 指令，这将强制 Redis 仅侦听 IPv4 和 IPv6（如果可用）环回接口地址（这意味着 Redis 将只能接受来自运行它的同一主机的客户端连接）。
# 如果您确定希望您的实例侦听所有接口，请注释掉以下行。
# 除非您明确禁用保护模式，否则您还需要设置密码。
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# bind 127.0.0.1 -::1

# 默认情况下，传出连接（从副本到主节点、从 哨兵 到实例、集群总线等）未绑定到特定的本地地址。
# 在大多数情况下，这意味着操作系统将根据路由和连接通过其的接口来处理该问题。
...
# 保护模式是一层安全保护，以避免互联网上开放的 Redis 实例被访问和利用。
#
# 当保护模式打开且默认用户没有密码时，服务器只接受来自 IPv4 地址 （127.0.0.1）、IPv6 地址 （：：1） 或 Unix 域套接字的本地连接。
#
# 默认情况下，保护模式处于启用状态。只有当您确定希望来自其他主机的客户端连接到 Redis 时，才应禁用它，即使未配置身份验证也是如此。
protected-mode no

# Redis 使用默认强化的安全配置指令来减少对无辜用户的攻击面。
# 因此，一些敏感的配置指令是不可变的，并且一些具有潜在危险的命令被阻止。
#
# 控制 Redis 写入的文件（例如，'dir' 和 'dbfilename'）且通常在运行时不修改的配置指令通过使它们不可变来保护。
#
# 默认情况下，可以增加 Redis 攻击面且用户通常不调用的命令将被阻止。
#
# 通过将下面列出的每个配置设置为以下任一值，可以将这些值公开给所有连接或仅本地连接：
#
# no    - 阻止任何连接（保持不可变）
# yes   - 允许任何连接（无保护）
# local - 仅允许本地连接。源自 IPv4 地址 （127.0.0.1）、IPv6 地址 （：：1） 或 Unix 域套接字的端口。
#
...
# 如果指定了 port 0，Redis 将不会监听 TCP 套接字。
port 6379

# TCP listen() 积累.
#
# 在每秒请求数较高的环境中，您需要大量的积累工作，以避免客户端连接速度缓慢的问题.
# 请注意，Linux 内核会以静默方式将其截断为 /proc/sys/net/core/somaxconn 的值，因此请确保同时提高 somaxconn 和 tcp_max_syn_backlog 的值，以获得所需的效果。
tcp-backlog 511
...
# 应用特定于操作系统的机制，使用指定的 ID 标记侦听套接字，以支持高级路由和筛选功能。
#
# 在 Linux 上，ID 表示连接标记。
# 在 FreeBSD 上，ID 代表套接字 cookie ID。
...
# 配置 X.509 证书和私钥，用于向连接的客户端、主节点或集群对等节点验证服务器。  这些文件应为 PEM 格式。
#
# tls-cert-file redis.crt
# tls-key-file redis.key
#
# 如果密钥文件使用密码加密，则也可以在此处包含密钥文件。
#
# tls-key-file-pass secret

# 通常，Redis 对服务器功能（接受连接）和客户端功能（从 master 复制、建立集群总线连接等）使用相同的证书。
#
# 有时，颁发证书时具有将其指定为仅限客户端或仅限服务器的证书的属性。在这种情况下，可能需要对传入 （服务器） 和传出 （客户端） 连接使用不同的证书。
# 为此，请使用以下指令：
#
...
# 配置 DH 参数文件以启用 Diffie-Hellman （DH） 密钥交换，这是旧版本的 OpenSSL （<3.0） 所必需的。较新的版本不需要此配置，建议不要使用此配置。
#
# tls-dh-params-file redis.dh

# 配置 CA 证书捆绑包或目录以对 TLS/SSL 客户端和对等体进行身份验证。
# Redis 需要其中至少一个的显式配置，并且不会隐式使用系统范围的配置。
#
...
#
# 如果指定了 “no”，则不需要也不接受客户端证书。
# 如果指定了 “optional”，则接受客户端证书，并且客户端证书必须有效（如果提供），但不是必需的。
#
# tls-auth-clients no
...
# 默认情况下，Redis 集群总线使用普通 TCP 连接。
# 要为 集群总线 协议启用 TLS，请使用以下指令：
#
# tls-cluster yes

# 默认情况下，仅启用 TLSv1.2 和 TLSv1.3，强烈建议将正式弃用的旧版本保持禁用状态，以减少攻击面。
# 您可以明确指定要支持的 TLS 版本。
...
# 配置允许的密码。 有关此字符串的语法的更多信息，请参见 ciphers（1ssl） 手册页。
#
# 注意：此配置仅适用于 <= TLSv1.2。
#
# tls-ciphers DEFAULT:!MEDIUM

# 配置允许的 TLSv1.3 密码套件。 有关此字符串的语法的更多信息，特别是 TLSv1.3 密码套件，请参见 ciphers（1ssl） 手册页。
#
# tls-ciphersuites TLS_CHACHA20_POLY1305_SHA256

# 选择密码时，请使用服务器的首选项，而不是客户端首选项。默认情况下，服务器遵循客户端的首选项。
#
# tls-prefer-server-ciphers yes

# 默认情况下，TLS 会话缓存处于启用状态，以允许支持它的客户端更快、更便宜地重新连接。使用以下指令禁用缓存。
#
...
# 更改缓存的 TLS 会话的默认超时。默认超时为 300 秒。
#
# tls-session-cache-timeout 60

################################# 通用 #####################################

# 默认情况下，Redis 不作为守护程序运行。如果需要，请使用 'yes'。
# 请注意，当守护进程化时，Redis 将在 /var/run/redis.pid 中写入一个 pid 文件。
...
# 选项:
#   supervised no      - 无监督交互
#   supervised upstart - 通过将 Redis 置于 SIGSTOP 模式来发出信号 upstart 需要在 upstart 作业配置中使用 “expect stop”
...
# 注意：这些监督方法仅表示“流程已准备就绪”。它们不会启用连续 ping 返回您的主管。
#
# 默认值为 “no”。要在 upstart/systemd 下运行，您只需取消注释下面的行即可：
#
# supervised auto

# 如果指定了 pid 文件，Redis 会在启动时将其写入指定位置，并在退出时将其删除。
#
# 当服务器以非守护进程运行时，如果在配置中未指定任何文件，则不会创建 pid 文件。当服务器被守护时，即使未指定，也会使用 pid 文件，默认为 “/var/run/redis.pid”。
#
# 创建 pid 文件是尽力而为：如果 Redis 无法创建它，没有发生任何不好的事情，服务器将正常启动和运行。
#
# 请注意，在现代 Linux 系统上，“/run/redis.pid” 更符合要求，应该改用。
...
# 指定日志文件名。此外，空字符串可用于强制 Redis 记录标准输出。请注意，如果您使用标准输出进行日志记录但使用 daemonize，则日志将发送到 /dev/null
logfile ""

# 要启用对系统记录器的日志记录，只需将“syslog-enabled”设置为 yes，并根据需要选择更新其他 syslog 参数。
# syslog-enabled no

# 指定 syslog 身份。
# syslog-ident redis

# 指定 syslog 工具。必须是 USER 或 LOCAL0-LOCAL7 之间。
# syslog-facility local0

# 要禁用内置的崩溃日志（这可能会在需要时生成更清晰的核心转储），请取消注释以下内容:
#
# crash-log-enabled no

# 要禁用作为崩溃日志的一部分运行的快速内存检查（这可能会让 redis 更早终止），请取消注释以下内容:
#
# crash-memcheck-enabled no

# 设置数据库的数量。默认数据库为 DB 0，您可以使用 SELECT 在每个连接的基础上选择不同的数据库<dbid>，其中 dbid 是介于 0 和 'databases'-1 之间的数字
databases 16

# 默认情况下，Redis 仅在开始记录到标准输出时显示 ASCII 艺术徽标，并且标准输出是 TTY 并且禁用了 syslog 日志记录。
# 基本上，这意味着通常 logo 只在交互式会话中显示。
# 但是，通过将以下选项设置为 yes，可以强制执行 4.0 之前的行为并始终在启动日志中显示 ASCII 艺术徽标。
always-show-logo no

# 为避免将个人身份信息 （PII） 记录到服务器日志文件中，请取消注释以下内容:
#
# hide-user-data-from-log yes

# 默认情况下，Redis 会修改进程标题（如 'top' 和 'ps' 中所示）以提供一些运行时信息。可以通过将以下内容设置为 no 来禁用此功能并将进程名称保留为已执行.
set-proc-title yes
...
# {listen-addr}     绑定地址或 '*'，后跟 TCP 或 TLS 端口侦听，或者如果可用，则为 Unix 套接字。
...
# 设置本地环境，用于字符串比较操作，也会影响 Lua 脚本的性能。Empty String 表示区域设置是从环境变量派生的。
locale-collate ""
...
# 工作目录。
# 数据库将写入此目录内，文件名在上面使用 'dbfilename' 配置指令指定。
# 仅附加文件也将在此目录中创建。
...
# 选项效果将只是为默认用户设置密码。
# 客户端仍将<password>像往常一样使用 AUTH 进行身份验证，或者更明确地使用 AUTH default（<password>如果遵循新协议），则两者都可以正常工作。
# requirepass 与 aclfile 选项和 ACL LOAD 命令不兼容，它们将导致 requirepass 被忽略。
...
# 在某些情况下，如果 redis 检测到系统处于不良状态，它会发出警告甚至拒绝启动，可以通过设置以下配置来抑制这些警告，该配置采用空格分隔的警告列表来抑制
#
