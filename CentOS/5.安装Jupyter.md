...
# 修改文件 找到c.NotebookApp.password 把之前复制的值粘贴进去
...
# 安装JupyterHub
sudo docker run -d -p 2345:8000 --name jupyterhub --restart=always -v /data/JupyterHub/home:/home -v /data/JupyterHub/jupyterhub:/srv/jupyterhub -v /etc/localtime:/etc/localtime jupyterhub/jupyterhub jupyterhub

# 进入容器
sudo docker exec -it jupyterhub bash

# 设置root密码 方便后续网页添加用户和密码 后续在终端中su root 切换后 adduser即可
passwd root
...
# 安装jupyterlab 记得查询linux中的python版本 安装对应的dev
...
c.JupyterHub.admin_users = {'之前添加的用户名'}

# 然后保存退出

sudo docker restart jupyterhub

# 现在默认使用 Notebook 所以需要改成JupyterLab
sudo docker exec -it jupyterhub bash

# 安装jupyterlab 之前安装了 所以不用安装
# 安装jupyterlab 记得查询linux中的python版本 安装对应的dev
apt-get install python3.8-dev
...
# 改成 c.Spawner.cmd = ['jupyterhub-singleuser'] 如下语句
